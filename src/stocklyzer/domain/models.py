...
class FinancialPeriod:
    ...
    def __post_init__(self):
        ...
        for field_name in ["total_revenue", "net_income", "total_assets", "total_liabilities", "total_equity",
                           "operating_cash_flow", "investing_cash_flow", "financing_cash_flow", "changes_in_cash",
                           "free_cash_flow"]:
            value = getattr(self, field_name)
            if value is not None:
...
class FinancialHistory:
    ...
    def get_financial_growth(self, period_type: str = "annual") -> Dict[str, List[Optional[Decimal]]]:
        """Calculate financial growth rates."""
        periods = self.annual_periods if period_type == "annual" else self.quarterly_periods
        return {
            "total_revenue": self._calculate_growth_rates(periods, "total_revenue"),
            "net_income": self._calculate_growth_rates(periods, "net_income")
        }

    def get_balance_sheet_growth(self) -> Dict[str, List[Optional[Decimal]]]:
        """Calculate balance sheet growth rates."""
        return {
            "total_assets": self._calculate_growth_rates(self.annual_periods, "total_assets"),
            "total_liabilities": self._calculate_growth_rates(self.annual_periods, "total_liabilities"),
            "total_equity": self._calculate_growth_rates(self.annual_periods, "total_equity")
        }
